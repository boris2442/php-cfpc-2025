...
if (!empty($_POST)) {
    if (
        ...
        if (empty($error)) {
            ...
            $requete->bindValue(':email', $email, PDO::PARAM_STR);
            $requete->bindValue(':password_hash', $password_hash, PDO::PARAM_STR); // Utilisez le mot de passe haché
            $requete->bindValue(':role', $role, PDO::PARAM_STR); // Utilisez un paramètre lié pour le rôle
            $requete->execute();
...
<form method="POST" action="" class=" bg-white p-6 rounded shadow max-w-lg mx-auto">
    <div class="flex flex-col gap-[7px] pt-[7px]">
        ...
        <div class="text-left flex flex-col gap-[7px]">
            ...
            <input type="text" placeholder="Votre mail" id="mail" value="<?php echo $mail ?? ""  ?>" name="email" class="w-full border border-green-300 p-2 rounded focus:outline-none focus:border-green-500" />
        </div>
